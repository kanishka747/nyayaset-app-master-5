...
// Dashboard Component - Add this before the main JurisConnect component
const Dashboard: React.FC = () => {
  const [checkedTasks, setCheckedTasks] = useState<Record<number, boolean>>({});

  const caseStats = [
    { label: 'Active Cases', value: '24', change: '+3', icon: Briefcase, color: 'from-blue-500 to-indigo-600', bg: 'from-blue-50 to-indigo-50' },
    { label: 'Won This Month', value: '12', change: '+5', icon: Trophy, color: 'from-green-500 to-emerald-600', bg: 'from-green-50 to-emerald-50' },
    { label: 'Pending Reviews', value: '8', change: '-2', icon: Clock, color: 'from-orange-500 to-red-600', bg: 'from-orange-50 to-red-50' },
    { label: 'Client Meetings', value: '15', change: '+4', icon: Users, color: 'from-purple-500 to-pink-600', bg: 'from-purple-50 to-pink-50' }
  ];

  const recentCases = [
    { id: '1', title: 'Contract Dispute - ABC Corp', client: 'ABC Corporation', status: 'In Progress', priority: 'High', nextHearing: 'Dec 15, 2024', progress: 65 },
    { id: '2', title: 'Property Rights Case', client: 'John Smith', status: 'Discovery', priority: 'Medium', nextHearing: 'Dec 20, 2024', progress: 40 },
    { id: '3', title: 'Employment Dispute', client: 'XYZ Ltd', status: 'Filing', priority: 'Low', nextHearing: 'Jan 05, 2025', progress: 25 }
  ];

  const upcomingTasks = [
    { task: 'File appeal in High Court', due: 'Today', priority: 'High' },
    { task: 'Client meeting - Smith case', due: 'Tomorrow', priority: 'Medium' },
    { task: 'Review contract documents', due: 'Dec 18', priority: 'Low' },
    { task: 'Prepare witness examination', due: 'Dec 20', priority: 'Medium' }
  ];
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-500 via-purple-600 to-pink-500 rounded-3xl p-8 text-white shadow-2xl">
        <h2 className="text-3xl font-black mb-2">Welcome back, John! 👋</h2>
        <p className="text-blue-100 font-semibold">Here's what's happening with your practice today</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-4 gap-6">
        {caseStats.map((stat, index) => (
          <div key={index} className={`bg-gradient-to-br ${stat.bg} rounded-3xl p-6 shadow-xl border border-gray-100 hover:shadow-2xl transition-all transform hover:-translate-y-1`}>
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-xs font-bold px-3 py-1 rounded-full ${stat.change.startsWith('+') ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                {stat.change}
              </span>
            </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        {/* Recent Cases */}
        <div className="col-span-2 bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-black text-gray-900 flex items-center gap-2">
              <Briefcase className="w-6 h-6 text-blue-500" />
              Recent Cases
            </h3>
            <button className="text-blue-600 font-bold text-sm hover:text-blue-700">View All</button>
          </div>
          <div className="space-y-4">
            {recentCases.map(caseItem => (
              <div key={caseItem.id} className="p-5 rounded-2xl bg-gradient-to-r from-gray-50 to-blue-50 border border-gray-100 hover:shadow-lg transition-all">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h4 className="font-bold text-gray-900 text-lg mb-1">{caseItem.title}</h4>
                    <p className="text-sm text-gray-600 mb-2">Client: {caseItem.client}</p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                    caseItem.priority === 'High' ? 'bg-red-100 text-red-600' :
                    caseItem.priority === 'Medium' ? 'bg-yellow-100 text-yellow-600' :
                ...
                <div className="flex justify-between items-center mb-3">
                  <span className="text-sm text-gray-600">Status: {caseItem.status}</span>
                  <span className="text-sm text-gray-600">Next: {caseItem.nextHearing}</span>
                </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
            ...
            <div className="space-y-4">
              {upcomingTasks.map((task, index) => (
                <div key={index} className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-gray-50 to-orange-50 border border-gray-100 hover:shadow-md transition-all">
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      <span className={`font-semibold ${checkedTasks[index] ? 'line-through text-gray-400' : 'text-gray-900'}`}>
                        {task.task}
                      </span>
              ...
              {performanceData.map((data, index) => (
                <div key={index} className="flex flex-col items-center flex-1">
                  <div 
                    ...
                    style={{ height: `${(data.cases / 15) * 100}%` }}
                  />
                  <span className="text-xs font-semibold text-gray-600 mt-2">{data.month}</span>
                  <span className="text-xs text-gray-500">{data.cases}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
        ...
        <div className="space-y-4">
          {[
            { action: 'Case Update', description: 'Filed motion in Smith vs. Jones', time: '2 hours ago', type: 'case' },
            { action: 'Document Review', description: 'Completed contract review for ABC Corp', time: '5 hours ago', type: 'document' },
            { action: 'Client Meeting', description: 'Scheduled with Johnson Enterprises', time: '1 day ago', type: 'meeting' },
            { action: 'Research Completed', description: 'Legal precedent research for patent case', time: '2 days ago', type: 'research' }
          ].map((activity, index) => (
            <div key={index} className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-gray-50 to-purple-50 border border-gray-100 hover:shadow-md transition-all">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                ...
                {activity.type === 'case' && <Gavel className="w-5 h-5 text-white" />}
                {activity.type === 'document' && <FileText className="w-5 h-5 text-white" />}
                {activity.type === 'meeting' && <Users className="w-5 h-5 text-white" />}
                {activity.type === 'research' && <BookOpen className="w-5 h-5 text-white" />}
              </div>
              <div className="flex-1">
...
// Add the Activity icon import at the top with other Lucide icons
// import { Activity, ... } from 'lucide-react';
